            ...
            <form method="POST" action="{{ route('survey.form.submit') }}">
                ...
                <div class="mb-3">
                    ...
                    <select name="jenis_instansi" class="form-select" id="jenis_instansi" required>
                        ...
                        <option value="pendidikan_tinggi" {{ old('jenis_instansi') == 'pendidikan_tinggi' ? 'selected' : '' }}>Pendidikan Tinggi</option>
                        <option value="instansi_pemerintah" {{ old('jenis_instansi') == 'instansi_pemerintah' ? 'selected' : '' }}>Instansi Pemerintah</option>
                        <option value="perusahaan_swasta" {{ old('jenis_instansi') == 'perusahaan_swasta' ? 'selected' : '' }}>Perusahaan Swasta</option>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="nama_instansi" class="form-control" id="nama_instansi" value="{{ old('nama_instansi') }}" required>
                    @error('nama_instansi')
                        <div class="text-danger mt-1">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="nim_mahasiswa" class="form-control" id="nim_mahasiswa" value="{{ old('nim_mahasiswa') }}" required>
                    @error('nim_mahasiswa')
                        <div class="text-danger mt-1">{{ $message }}</div>
                    @enderror
                </div>
                        ...
                        <div class="d-flex flex-column">
                            @foreach ($options as $option)
                                <div class="form-check">
                                    <input class="form-check-input" type="radio" name="{{ $key }}" value="{{ $option }}" {{ old($key) == $option ? 'checked' : '' }} required>
